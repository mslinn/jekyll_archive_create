...
class MakeArchive < Jekyll::Generator
  ...
  def setup_instance_variables(config)
    ...
    Jekyll.info "@archive_name=#{@archive_name}; @live_reload=#{@live_reload}; @force_delete=#{@force_delete}; @archive_files=#{@archive_files}"
  end
  ...
  def create_archive(source)
    ...
    Jekyll.info "#{archive_name_full} exists? #{archive_exists}"
    if archive_exists && @force_delete
      Jekyll.info "Deleting old #{archive_name_full}"
      File.delete(archive_name_full)
    end

    make_tar_or_zip(archive_exists, archive_name_full, source)

    Jekyll.info "Looking for #{@archive_name} in .gitignore..."
  ...
  def make_tar(tar_name, source)
    Dir.mktmpdir do |dirname|
      @archive_files.each do |filename|
        ...
        Jekyll.info "Copying #{filename_full} to temporary directory #{dirname}; filename=#{filename}; fn=#{fn}"
  ...
  def make_tar_or_zip(archive_exists, archive_name_full, source)
    if !archive_exists || @force_delete
      Jekyll.info "Making #{archive_name_full}"
      case @archive_type
      when :tar
  ...
  def make_zip(zip_name, source)
    ...
    Zip::File.open(zip_name, Zip::File::CREATE) do |zipfile|
      @archive_files.each do |filename|
        ...
        Jekyll.info "make_zip: adding #{filename_original} to #{zip_name} as #{filename_in_archive}"
  ...
  def qualify_file_name(path, source)
    ...
    when "/" # Is the file absolute?
      Jekyll.info "Absolute filename: #{path}"
    ...
    when "!" # Should the file be found on the PATH?
      ...
      Jekyll.info "File on PATH: #{clean_path} -> #{filename_full}"
